...
Sub-milestone patch on top of v0.6.  Closes all 9 audit pass-16 findings (1 must-fix ship-blocker + 1 must-fix defense-in-depth + 6 deferrable-to-v0.6.1 + 1 test-coverage gap) in a single release; every finding lands with a matching test so a future refactor that regresses one path breaks the corresponding test.

All 497 tests pass (487 pre-existing + 10 new); `test_no_forbidden_calls.py` still green; ruff clean on every changed file (pre-existing SIM102 in `organize/mover.py:143` unchanged, documented in v0.4); mypy `--strict` clean on every changed source file modulo the same pre-existing `unused-ignore` warnings on 3rd-party stubs (googleapiclient / osxphotos / tenacity / httpx) documented in v0.2 sub-milestone 2 and v0.5-a.

Shipped:

- `src/duplicate_cleaner/score/rules.py` (M1, must-fix) — the `is_shared → is_informational=True` marking block was extended to also fire for `source_id.startswith(("gphotos:", "icloud:"))`.  Symmetric to the shared-cloud invariant and to the two read-only invariants added in v0.6: a Google Photos or iCloud Photos member cannot become a discard candidate at scoring time, so `dc apply --commit` no longer refuses the whole run on any mixed local + gphotos/icloud scan.  Was: the scorer applied `cloud_when_local_exists=-3` and picked the read-only member as the discard; mover pre-flight then aborted the run.  Now: the read-only member is informational-only; the local peer stays keeper; apply commits.
- `src/duplicate_cleaner/sources/iclouddrive_photos.py::read_bytes` (M2, must-fix) — defense-in-depth against a tampered Photos database.  Before opening the local file, `read_bytes` now `resolve_for_check`s the path, runs `validate_not_excluded`, and requires containment inside the library bundle's parent directory (typically `~/Pictures`).  A hostile SQLite blob pointing at `/etc/passwd` or `~/.ssh/id_rsa` is refused with a typed `SourceError` — no bytes are read into the BLAKE3 cache.
- `src/duplicate_cleaner/paths.py::_PROVIDER_ID_PATTERNS` (M3) — added `gphotos` and `icloud` entries so `validate_cloud_entry_with_authorized` no longer silently skips the `cloud_file_id` shape check on either new provider.  The gphotos regex mirrors `sources.gphotos._GPHOTOS_ID_RE`; icloud accepts `^[A-Za-z0-9-]{20,}$` for the uuid shape.
- `src/duplicate_cleaner/sources/gphotos.py::read_bytes` (M4) — the CDN stream call is now wrapped in a tenacity `Retrying` loop over 429 / 5xx / `httpx.TransportError`.  Google's storage host throttles large libraries with 429s the same way the Photos API does; before the patch a full reconcile on a 50k+ library silently dropped records on transient hiccups.  Symmetric to the retry pattern in `sources/onedrive.py::_is_retryable_http_error`.
- `src/duplicate_cleaner/sources/gphotos.py::read_bytes` (M5) — content-type + empty-body sanity guards.  A 200 response is now validated against `content-type` starting with `image/` or `video/`; the total byte count is verified >0 after the stream drains.  A session-expired redirect returning an HTML interstitial can no longer be hashed as image bytes and cluster into a fake duplicate group.
- `src/duplicate_cleaner/sources/iclouddrive_photos.py::_open_db` (M6) — the `except ImportError` narrow was broadened to also cover `(OSError, PermissionError, sqlite3.DatabaseError)`.  A macOS host without Full Disk Access granted to Terminal.app now surfaces a typed `SourceError` with the actionable FDA hint ("Grant Full Disk Access to Terminal.app in System Settings → Privacy & Security → Full Disk Access") instead of a raw permission stack trace.
- `src/duplicate_cleaner/sources/gphotos.py::move_to_trash` (M7, simplification) — dropped the redundant `if self.is_read_only_scan: raise PermissionError(...)` branch.  Every apply-time construction site (`cli._build_sources_for_apply`) hard-codes `is_read_only_scan=True`, so the branch was unreachable in production; the method now unconditionally raises `SourceError("Google Photos trash deferred to v0.6.1; scope escalation required")`, matching the simpler iCloud Photos shape.  v0.6.1 will reintroduce a flag-gated write path when scope escalation actually flips.
- `src/duplicate_cleaner/sources/iclouddrive_photos.py::list_files` (M9) — the "skipped N stubs" warning was wrapped in try/finally so it fires even when the caller closes the generator early.  Before v0.6-patch a `dc auth test icloud:*` call (reads only the first record via `next(iter(...))`) never surfaced the warning; users with hundreds of stubs and one downloaded photo saw "OK: first photo …" with no hint that "Optimize Mac Storage" was hiding the rest of the library.

Tests shipped (10 net-new):

- `tests/test_scoring_cross_source.py` (2 tests): `test_gphotos_source_marked_informational`, `test_icloud_source_marked_informational` — the M1 fix, locked in symmetrically with the existing `test_shared_cloud_file_is_informational`.
- `tests/test_sources_icloud.py` (3 tests): `test_icloud_read_bytes_refuses_excluded_path` (M2 — mocks a poisoned `photo.path='/etc/hosts'` → SourceError), `test_open_db_permission_denied_raises_source_error_with_hint` (M6 — asserts the FDA hint appears in the message), `test_stub_warning_fires_on_early_close` (M9 — closes the generator after one record, asserts the warning was logged).
- `tests/test_mover_source_id_dispatch.py` (1 test): `test_gphotos_cloud_file_id_shape_validated_by_mover` (M3 — poisoned `cloud_file_id="../evil/foo"` on gphotos entry → mover raises `ApplyError` at pre-flight).
- `tests/test_sources_gphotos.py` (2 tests): `test_read_bytes_retries_cdn_429` (M4 — httpx 429 twice then 200, verify stream succeeds and retries fired), `test_read_bytes_refuses_html_response` (M5 — 200 with `content-type: text/html` → SourceError).
- `tests/test_apply_cloud_dispatch.py` (2 tests): `test_apply_refuses_gphotos_discards`, `test_apply_refuses_icloud_discards` (M8 — hand-crafted report with a gphotos / icloud discard, verify `apply_report(commit=True)` raises `ApplyError` at the read-only pre-flight tripwire; source's `check_drift` / `move_to_trash` are never called).

Existing test updates:

- `tests/test_sources_gphotos.py::test_move_to_trash_raises_when_read_only` — was: asserted `PermissionError` from the dropped read-only tripwire.  Now: asserts `SourceError` from the unified deferral message (M7).
- `tests/test_sources_gphotos.py::test_read_bytes_refetches_base_url` — the `_FakeStream` mock gained a `headers = {"content-type": "image/heic"}` attribute so the M5 content-type gate accepts it.

Invariants preserved:

- 487 pre-existing tests still pass unchanged (10 new tests added, total 497).
- `test_no_forbidden_calls.py` still green; `shutil.move` still confined to `apply/undo.py` + `organize/undo.py`.
- Cloud paths still never `.resolve()`d.
- BYO OAuth only for Google Photos; iCloud has no OAuth surface.
- Both v0.6 invariants ("Google Photos read-only in v0.6" + "iCloud Photos read-only permanently") upheld end-to-end — the M1 fix strengthens the scorer half of the safety envelope; M7 unifies the source-side refusal shape.

**v0.6-patch clears ship.**  All 9 pass-16 findings closed.  Ready for v0.6.1 (GPhotos trash scope escalation) or v0.7 (image near-dup).

### v0.6 — combined Code Review + Security pass 16 (2026-09-11)

**Sixteenth audit** — verdict: **Ready with must-fix follow-ups.** 9 findings total: 2 safety (PLAUSIBLE), 4 correctness (2 CONFIRMED / 2 PLAUSIBLE), 2 simplification / test-coverage (CONFIRMED), 1 UX-shaped correctness (CONFIRMED).  Zero DATA-LOSS committed; both v0.6 invariants ("Google Photos read-only in v0.6" + "iCloud Photos read-only permanently") upheld end-to-end.

Verified positives (no drift from prior invariants):
- `is_read_only_scan=True` default on both new sources; `GooglePhotosSource.move_to_trash` fires the read-only tripwire (PermissionError) BEFORE reaching the deferred-SourceError branch; `iCloudPhotosSource.move_to_trash` refuses unconditionally (SourceError, permanent read-only) regardless of the flag's value.
- `_build_sources_for_apply` (cli.py:1245) and `_build_scan_sources` (cli.py:1151) BOTH construct GooglePhotosSource with `is_read_only_scan=True` — apply-time construction cannot be tricked into flipping the flag off, so a hostile plan cannot reach the SourceError deferral branch.
- `apply/mover.py:501` pre-flight refuses the whole run when ANY cloud entry's source has `is_read_only_scan=True` — atomic refusal, no local moves fire either.
- `read_bytes` on Google Photos uses an UNAUTH `httpx.Client` with `follow_redirects=True`; no `Authorization` header is set on the client, so no bearer leaks even if the CDN redirects.  The Photos API bearer lives only on the `service` credentials object.
- `read_bytes` refetches the mediaItem via `mediaItems.get` on EVERY call (line 288) — not just first — so a stale `baseUrl` from a scan-time list is never streamed.
- `=d` original-quality suffix present on the CDN URL (gphotos.py:307).
- `_gphotos_call` covers 429/5xx via tenacity for the Photos API path (list_files + drift-check + read_bytes' `mediaItems.get`); `_raise_mapped` translates 401/403/404/429/5xx to typed errors symmetric with gdrive.
- Cloud media-item id shape gate (`_validate_media_item_id`, ^[A-Za-z0-9_-]{20,}$) fires BEFORE URL interpolation in both `read_bytes` and `check_drift`.
- iCloud stub records (`photo.path is None`) are NOT emitted; `stub_count` is incremented and a warning logs the hint pointing at "Download Originals to This Mac".
- iCloud + gphotos `move_to_trash` / `restore_from_trash` / `upload` all raise the correct typed error (SourceError for the deferred/refused paths; NotImplementedError for upload where the Source protocol default surfaces loudly).
- Cloud `Path` never `.resolve()`d in either new module; `test_no_forbidden_calls.py` still green over the new files (grep confirms zero forbidden calls).
- `shutil.move` still confined to `apply/undo.py` + `organize/undo.py`.
- Pagination via `nextPageToken` handles last-page-with-zero-items correctly (loop exits on missing/empty token).

Must-fix in same release (v0.6.1 or v0.6-patch):

- correctness/CONFIRMED (workflow gap contradicting the docs): `score/rules.py::_score_group` does NOT mark records with `source_id.startswith("gphotos:")` or `source_id.startswith("icloud:")` as `is_informational=True`, while `is_shared` cloud records ARE so marked (line 253).  The scorer then applies `cloud_when_local_exists=-3` to the read-only records (line 348), so on any mixed local+gphotos/local+icloud scan the read-only records score lower than the local peer and are proposed as the discard.  `dc apply --commit` then hits the pre-flight tripwire at `apply/mover.py:501` and refuses THE ENTIRE RUN — not per-entry.  `docs/cloud-oauth-setup.md:216` claims "`dc apply` refuses to trash iCloud Photos entries with a message pointing you at the Photos.app" which implies per-entry refusal; the actual behaviour is whole-run abort.  Result: any user who scans local + gphotos/icloud can never `dc apply --commit` without hand-editing report.json.  Fix: mirror the `is_shared → is_informational=True` block for `source_id` starting with `gphotos:` or `icloud:` (the invariant already forbids trashing them, so they can never be a legitimate discard).  Symmetric to the "Shared cloud files informational-only" invariant.
- safety/PLAUSIBLE (defense-in-depth): `sources/iclouddrive_photos.py::read_bytes` (line 222-223) opens `Path(path_raw)` directly with no validation against `EXCLUDED_ROOTS`, no symlink resolution + re-check, and no containment check against the library bundle.  `path_raw` comes from the Photos SQLite database inside the `.photoslibrary` bundle — under normal use this is Photos.app-managed, but if the DB is corrupt or tampered (the bundle is a plain directory on disk with no signature) a `photo.path` value pointing at `/etc/passwd`, `~/.ssh/id_rsa`, or a symlink into `/System` would be opened by the reconciler and hashed into the report.  Not DATA-LOSS (the source refuses move_to_trash), but a hostile Photos database could exfiltrate arbitrary bytes into the BLAKE3 cache / signal side-tables.  Fix: `resolve_for_check(local_path)` → `validate_not_excluded(resolved)` → `is_within(resolved, self._library_path)` inside `read_bytes` (and defensively in `list_files` before `stat`).  Mirrors the walker's excluded-roots rail on real-filesystem paths.

Deferrable to v0.6.1 (post-release notes):

- correctness/CONFIRMED (missing shape gate): `paths.py::_PROVIDER_ID_PATTERNS` has entries for `gdrive` + `onedrive` but NOT `gphotos` / `icloud`.  `validate_cloud_entry_with_authorized` therefore silently skips the `cloud_file_id` shape check for either new provider — a poisoned `report.json` with `cloud_file_id="../evil"` on a gphotos entry passes mover-level validation.  The GooglePhotosSource itself has a shape gate in `_validate_media_item_id` (defense in depth catches it before URL interpolation), and iCloud never round-trips the id to a URL, so no active data-integrity risk in v0.6.  Once v0.6.1 wires trash for gphotos and the poisoned-id reaches an actual API URL, the mover-level gate must be in place.  Fix: add `_GPHOTOS_CLOUD_FILE_ID_RE` (mirroring the source-side pattern) + `_ICLOUD_CLOUD_FILE_ID_RE` (accept `^[A-Za-z0-9-]{20,}$` for the uuid shape) to `_PROVIDER_ID_PATTERNS`.
- correctness/PLAUSIBLE (retry gap on CDN download): `sources/gphotos.py::read_bytes` wraps only the `mediaItems.get(...)` call in `_gphotos_call` — the httpx `cli.stream("GET", original_url)` that actually downloads the bytes is NOT retried on 429 / 5xx from the storage host.  Google Photos' CDN throttles large accounts (50k+ photos) with 429s the same way the API does, so a full reconcile pass on a large library sees per-photo failures that the reconciler surfaces as "reconcile stream failed" log lines and drops the record.  Fix: wrap the stream call in a tenacity retry over `httpx.HTTPStatusError`/429/5xx, or precheck `resp.status_code` inside `_gphotos_call`.
- correctness/PLAUSIBLE (missing content-type / empty-body guard): `sources/gphotos.py::read_bytes` (line 313-322) checks only `status >= 400`.  If the CDN returns 200 with an HTML error page (a session-expired redirect returning HTML to a browser context, or a proxy interstitial), the code hashes HTML instead of image bytes.  Every such record clusters into the "BLAKE3 of that HTML" group and is reported as a duplicate group.  Similarly, `iter_bytes` may yield zero chunks — the reconciler then computes the well-known BLAKE3 of empty (`af1349b9…`) and clusters every empty-body record as duplicates of empty-file records.  Fix: check `resp.headers.get("content-type", "").split("/")[0] == "image"` (or "video" for videos), OR require `content-length > 0`, OR track cumulative bytes and error on zero-length after iteration.
- correctness/CONFIRMED (typed-error mapping gap): `sources/iclouddrive_photos.py::_open_db` (line 110) catches only `ImportError`.  `osxphotos.PhotosDB(dbfile=...)` on a locked / corrupt / permission-denied SQLite database raises `sqlite3.OperationalError`, `PermissionError`, or `osxphotos`-defined exceptions unwrapped.  A user without Full Disk Access granted to Terminal (which the docs mention at cloud-oauth-setup.md:199) sees a raw permission error stack trace instead of "Grant Full Disk Access in System Settings → Privacy & Security".  Fix: broaden the `except` to `(ImportError, OSError, PermissionError)` and re-raise as `SourceError` with the Full-Disk-Access hint; document the osxphotos-specific errors in the same block.
- simplification/CONFIRMED (redundant tripwire): `sources/gphotos.py::move_to_trash` (line 330) checks `if self.is_read_only_scan: raise PermissionError(...)` BEFORE the deferred SourceError, but `_build_sources_for_apply` (cli.py:1245) hard-codes `is_read_only_scan=True` for gphotos — there is no production path in v0.6 where `is_read_only_scan=False` is reachable.  The two layers are inconsistent: the docstring says "Even before the scope-escalation guard, defense-in-depth" but the outer construction guarantees the flag never flips off.  Either the outer construction should let the flag flip (so the tripwire has a real purpose) or the tripwire should be dropped in favour of an unconditional SourceError.  Preferred: unconditional SourceError until v0.6.1 wires trash — matches iCloud Photos' shape (permanently unconditional).
- test-coverage/CONFIRMED: no test covers the apply-time integration for gphotos/icloud discards.  The v0.6 test suite exercises the source layer + auth registration + BYO refusal, but not the end-to-end `dc scan --sources local,gphotos:x` → `dc apply --commit` refusal path.  A future refactor that flipped `is_read_only_scan=True` to `False` in `_build_sources_for_apply` would not fail any test.  Add: `test_apply_refuses_gphotos_discards_via_read_only_tripwire` + `test_apply_refuses_icloud_discards_via_read_only_tripwire`.
- correctness/PLAUSIBLE (stub-count log fires late): `sources/iclouddrive_photos.py::list_files` (line 181) logs the warning ONLY after the generator is exhausted.  `dc auth test icloud:*` (cli.py:1902) reads only the FIRST record via `next(iter(...))`, so a library with hundreds of stubs and one downloaded photo would print "OK: first photo …" and never surface the stub count.  Fix: also log at first-record-yield if `stub_count > 0` at that point, or move the log into a `try/finally` inside `list_files` so it fires even when the generator is closed early.

Ship-readiness statement: zero DATA-LOSS committed, both v0.6 invariants upheld structurally (source layer + apply-time construction + pre-flight tripwire — three layers deep).  The two must-fix items are workflow / defense-in-depth issues:

1. **Scorer marking** — the docs promise per-entry refusal, but the mover does whole-run refusal, and the fix (mark gphotos/icloud as informational) is a 5-line change symmetric to the existing `is_shared` block.  Without it, v0.6's headline mixed-source scan is effectively unusable with `--commit`.
2. **Path validation on iCloud read** — defense-in-depth against a tampered Photos database.  Low-severity in practice (no move_to_trash on this source) but the walker's EXCLUDED_ROOTS rail is a core invariant of the project and should apply symmetrically here.

Both are 1-day fixes.  Recommendation: **ship v0.6 as-is IF the scorer marking finding is patched in a v0.6-patch** (correctness/workflow); the other items can defer to v0.6.1 alongside the trash scope escalation.
