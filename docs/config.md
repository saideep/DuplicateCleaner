...
Editing `weights.json` by hand is supported. Values are clamped to a sane range at load time so no single signal can dominate. Adaptive weight learning ships in v0.6 — until then, weights only change when you edit the file or run `dc weights reset`.
